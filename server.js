...
app.get("/services/black-car-limo-memphis/", (req, res) => {
    let html = fs.readFileSync(
        path.join(
            __dirname,
            "public/services/black-car-limo-memphis/index.html"
        ),
        "utf-8"
    )
    html = html.replaceAll("{{nonce}}", res.locals.nonce)
...
app.get("/services/airport-transfer-memphis/", (req, res) => {
    let html = fs.readFileSync(
        path.join(
            __dirname,
            "public/services/airport-transfer-memphis/index.html"
        ),
        "utf-8"
    )
    html = html.replaceAll("{{nonce}}", res.locals.nonce)
...
app.get("/services/corporate-travel-memphis/", (req, res) => {
    let html = fs.readFileSync(
        path.join(
            __dirname,
            "public/services/corporate-travel-memphis/index.html"
        ),
        "utf-8"
    )
    html = html.replaceAll("{{nonce}}", res.locals.nonce)
...
app.get("/services/event-transportation-memphis/", (req, res) => {
    let html = fs.readFileSync(
        path.join(
            __dirname,
            "public/services/event-transportation-memphis/index.html"
        ),
        "utf-8"
    )
    html = html.replaceAll("{{nonce}}", res.locals.nonce)
...
app.get("/services/long-distance-memphis/", (req, res) => {
    let html = fs.readFileSync(
        path.join(
            __dirname,
            "public/services/long-distance-memphis/index.html"
        ),
        "utf-8"
    )
    html = html.replaceAll("{{nonce}}", res.locals.nonce)
...
async function formatAndSendEmail(req, res) {
    ...
    if (!outcome.success) {
        ...
        if (formData["pickup-date-time"]) {
            formData["pickup-date-time"] = new Date(
                formData["pickup-date-time"]
            ).toLocaleString()
        }

        if (formData["dropoff-time"]) {
            formData["dropoff-time"] = format24HourTime(
